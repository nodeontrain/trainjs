	...
	if (filename == "application") {
		...
		input.on('data', function(data) {
			...
			while (index > -1) {
				...
				if (line.split('//=')[1]) {
					var require_str = jtrain_trim(line.split('//=')[1]);
...
stylesheet_link_tag = function(filename, options) {
	...
	if (filename == "application") {
		var filepath = train_root_app + '/app/assets/stylesheets/application.css';
		var input = fs.createReadStream(filepath);
		var remaining = '';
		var html = "";
		var fiber = Fiber.current;
			...
			while (index > -1) {
				...
				if (line.split('*=')[1]) {
					...
					if (require_str == "require_tree .") {
						css_files = fs.readdirSync(train_root_app + '/app/assets/stylesheets');
