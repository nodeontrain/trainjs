			...
			if (req_parser.controller == "root") {
				...
				if (trainRoutes['create'][controller_url]) {
					...
					for(var k = 0; k < len - 1; k++)
						controller_url = controller_url + urlarr[k];
					params.controller_url = controller_url;
					params['id'] = urlarr[len - 1];
					params['controller'] = urlarr[len - 2];
					routesFunc('update', params.controller_url);
				}
				...
				params['id'] = urlarr[len - 1];
				params['controller'] = urlarr[len - 2];
				routesFunc('destroy', params.controller_url);
				...
				if (urlarr[len - 1] == "new") {
					...
					params.controller_url = controller_url;
					params['controller'] = urlarr[len - 2];
					routesFunc('new', params.controller_url);
				} else if (urlarr[len - 1] == "edit") {
					//URL: hostname/controllers/:id/edit controllers#edit
					params['controller'] = urlarr[len - 3];
					params['id'] = urlarr[len - 2];
					for(var k = 0; k < len - 2; k++)
						controller_url = controller_url + urlarr[k];
					params.controller_url = controller_url;
					routesFunc('edit', params.controller_url);
				} else {
					...
					if (trainRoutes['index'][controller_url + parts[0]]) {
						...
						params.controller_url = controller_url + parts[0];
						routesFunc('index', params.controller_url);
					} else {
						//URL: hostname/controllers/:id or hostname/controllers/:id.json controllers#show
						params['id'] = parts[0];
						params['controller'] = urlarr[len - 2];
						params.controller_url = controller_url;
						routesFunc('show', params.controller_url);
					}
				}
			}
		}
