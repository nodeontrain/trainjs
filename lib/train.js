...
var initServer = function() {
	...
	CONFIG = { initializers: {} };
	CONFIG.env = { TRAINJS_ENV: 'development' };

	if (fs.existsSync(ROOT_APP + '/config/environment.js')) {
		var env = require(ROOT_APP + '/config/environment.js');
		for (var k in env) {
			CONFIG.env[k] = env[k];
		}
	}
	...
	if ( fs.existsSync(ROOT_APP + '/config/database.json') && fs.existsSync(ROOT_APP + '/node_modules/sequelize/lib/sequelize.js') ) {
		...
		if ( env_param_index > -1 && process.argv[ env_param_index + 1 ] ) {
			CONFIG.env.TRAINJS_ENV = process.argv[ env_param_index + 1 ];
		}
		var config = require(ROOT_APP + '/config/database.json')[CONFIG.env.TRAINJS_ENV];
