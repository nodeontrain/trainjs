			...
			if (routes[k]['resources']) {
				...
				if (!routes[k]['action'] && url_path.length > 2) {
					...
					if (method == 'get' && url_path.length > 2)
						action = 'show'
					else if (method == 'post' && url_path.length > 1)
						action = 'create'
					else if (method == 'put' && url_path.length > 2)
						action = 'update'
					else if (method == 'delete' && url_path.length > 2)
						action = 'destroy'
				}
				...
				if (routes[k]['except_before_filter'])
					before_filter[method + url] = true;
				else
					before_filter[method + url] = false;
