...
function newServer() {
	...
	if ( fs.existsSync(ROOT_APP + '/config/database.js') )
		CONFIG.database = require(ROOT_APP + '/config/database.js');
	...
	for (var f_i = 0; f_i < initializers_files.length; f_i++) {
		var path = initializers_files[f_i].split('/');
		var initializer_name = path[ path.length - 1 ];
		var initializer = initializer_name.split('.js')[0];
		CONFIG.initializers[initializer] = require(initializers_files[f_i]);
	}

	var controllers = {};
	var files = listFiles(ROOT_APP + '/app/controllers');
	for (var f_i = 0; f_i < files.length; f_i++) {
		var path = files[f_i].split('/');
		var controller_name = path[ path.length - 1 ];
		var resource = controller_name.split('_controller')[0];
		controllers[resource] = require(files[f_i]);
	}

	var model_files = listFiles(ROOT_APP + '/app/models');
	for (var f_i = 0; f_i < model_files.length; f_i++) {
		var path = model_files[f_i].split('/');
		...
		global[model] = require(model_files[f_i]);
	}
