...
var response_assets = function (res, req_parser, asset_path) {
	var type = req_parser.content_type;
	var path = asset_path;
	if (type == "text/html")
		path = req_parser.root_path;

	fs.readFile(path, function(err, data) {
		if (err && err.code == "ENOENT" && type == "text/html")
	...
	jroad.req_parser(req, function (req_parser) {
		...
		if (f && f == 'jtrain_ujs.js') {
			...
			if (typeof req_parser.root_path !== "undefined")
				response_assets(res, req_parser);
			else
