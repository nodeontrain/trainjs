...
function train_render(res, req_parser, obj, ext) {
	...
	fs.readFile(file, function(err_fs, data) {
		...
		if (typeof req_parser['train_notice'] !== "undefined" && req_parser['train_notice'] == "yes"
			&& typeof train_notice[req_parser.controller_url] !== "undefined") {
			console.log(train_notice[req_parser.controller_url]);
			for(var k in train_notice[req_parser.controller_url]) {
				obj[k] = train_notice[req_parser.controller_url][k];
			}
		}
...
routesFunc = function (res, req_parser) {
	...
	controllerClass.prototype.redirect_to = function(options, response_status) {
		if (typeof options !== "object") {
			...
			train_notice[request_parser.controller_url] = response_status;
			var lnk = options;
		}
