...
exports.router = function (app, router) {
    for (var route in Application.routing) {
        if (route == "resources") {
            Application.routing[route].forEach(function(resource) {                
                router.get("/" + resource, app.use(function(req, res, next) {
                    console.log(req.body);
                    res.end(resource + " index");
                }));
                router.get("/" + resource + "/:id", app.use(function(req, res, next) {
                    res.end(resource + " show");
                }));
                router.get("/" + resource + "/new", app.use(function(req, res, next) {
                    res.end(resource + " new");
                }));
                router.get("/" + resource + "/:id/edit", app.use(function(req, res, next) {
                    res.end(resource + " edit");
                }));
                router.post("/" + resource, app.use(function(req, res, next) {
                    res.end(resource + " create");
                }));
                router.put("/" + resource + "/:id", app.use(function(req, res, next) {
                    res.end(resource + " update");
                }));
                router.delete("/" + resource + "/:id", app.use(function(req, res, next) {
                    res.end(resource + " destroy");
                }));
            });
        }
    }
...
var routing = function(req, res, match) {
    var m = match.next();
    console.log(m);
}
