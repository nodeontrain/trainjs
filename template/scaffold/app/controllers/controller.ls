class App.%%controller_name%%Controller extends ApplicationController
	...
	show: !->
		%%model%% = %%model_name%%.find_by_id(this.params['id'])
		App.respond_to do
	...
	create: !->
		...
		.error (error) !->
			App.respond_to do
				format_html: render action: "new", data: { %%model%% : %%model%%, error: error }
				format_json: render json: error, status: "unprocessable_entity"

	# PUT /%%model_plural%%/1
	# PUT /%%model_plural%%/1.json
	update: !->
		%%model%% = %%model_name%%.find_by_id(this.params['id'])
		%%model%%..updateAttributes(this.params['data']).success !->
			App.respond_to do
				format_html: redirect_to %%model%%_path(%%model%%), notice: "%%model_name%% was successfully updated."
				format_json: { head: "no_content" }
		.error (error) !->
			App.respond_to do
				format_html: render action: "edit", data: { %%model%% : %%model%%, error: error }
				format_json: render json: error, status: "unprocessable_entity"
